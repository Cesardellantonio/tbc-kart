...
// visual ride (lift, rib-to-rib hop, tilt toward the kerb side). Camera and audio read amount / hz / phase.
...
export class KerbFeel {
  ...
  update(kart, path, curbs, index, dt) {
    ...
    this.phase = (this.phase + this.hz * dt * Math.PI * 2) % (Math.PI * 2);
  ...
  _ride(model, speed) {
    ...
    const rib = Math.sin(this.phase) * 0.5 + 0.5;
    ...
    model.body.rotation.x += a * KERB_RIDE.hop * pace * Math.sin(this.phase * 0.5 + 1.1);
  }
}
