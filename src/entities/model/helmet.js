// Full-face karting helmet (shape in helmetShell.js): a level-hemmed shell with a deep chin bar, a
// tinted visor on side pivots under a peak, vents, a livery stripe and the neck roll inside the hem.
...
function visor(mat) {
  ...
  // Peak: a lip along the visor's top edge, standing proud at its lower edge, which bows down a little
  const bow = (u, y) => (y < 0.01 ? 0.005 * (1 - u * u) : 0);
  const peakPt = (u, y, lift) =>
    at(u * 0.55, visorTop((u * 0.55) / VISOR_AZ) + y - bow(u, y), y < 0.01 ? lift : lift * 0.1);
...
function stripe(mat) {
  ...
  const band = range(12, 1.75, 2 * Math.PI - 1.75)
    .map((az) => [0.014, 0.036].map((h) => at(az, hemY(az) + h, 0.0015)));
...
export function buildHelmet(mat) {
  ...
  return g; // the neck under it is part of the suit (driver.js), so no dark stalk shows below the chin
}
