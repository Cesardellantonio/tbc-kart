// Cockpit parts: bucket seat, fuel tank between the legs, pedals, steering column and the wheel
// (three spokes, padded grips at quarter to three) on its own group so it can turn.
...
import { BOOT_PITCH } from './boot.js';
...
// Pedal plates lie along the boot soles (suit.js puts each heel on the tray at z −0.5)
function pedals(mats) {
  const out = [];
  for (const s of [-1, 1]) {
    const plate = new THREE.Mesh(new THREE.BoxGeometry(0.055, 0.12, 0.012), mats.frame);
    plate.position.set(s * 0.105, 0.134, -0.649);
    plate.rotation.x = BOOT_PITCH - Math.PI / 2; // square to the sole
    out.push(plate, rod(v3(s * 0.105, 0.1, -0.63), v3(s * 0.105, 0.055, -0.58), 0.008, mats.frame));
  }
  return out;
}
...
function steeringWheel(mats) {
  ...
  wheel.add(new THREE.Mesh(new THREE.TorusGeometry(R, 0.012, 6, 24), mats.accent));
...
export function buildCockpit(mats) {
  ...
  const [foot, mid] = [-0.44, -0.2].map((k) => hub.clone().addScaledVector(axis, k));
  const parts = [seat(mats.seat), ...tank(mats), ...pedals(mats), rod(hub, foot, 0.012, mats.frame, 8)];
  for (const s of [-1, 1]) parts.push(rod(mid, v3(s * 0.12, 0.058, -0.2), 0.008, mats.frame));
